...
const FREE_FEEDS: FeedType[] = [
  ...
  },
  {
    ...
    tierCost: "Free",
    icon: "🐘",
    description: "Search accounts across Mastodon instances. Provide your instance URL and optional access token.",
    authFields: [
      { key: "api_key", label: "Access Token (optional)", type: "password", required: false,
        help: "Optional. Allows higher rate limits on your instance." },
    ],
    settingsFields: [
      SEARCH_QUERIES_FIELD,
      { key: "instance_url", label: "Instance URL", type: "text", required: true,
        placeholder: "https://mastodon.social", default: "https://mastodon.social" },
    ],
    defaultIntervalMins: 60,
    minIntervalMins: 30,
  ...
  },
  {
    ...
    tierCost: "Free",
    icon: "📡",
    description: "Monitor RSS or Atom feeds for handle mentions. Useful for news aggregators and platform changelog feeds.",
    authFields: [],
    settingsFields: [
      { key: "feed_urls", label: "Feed URLs", type: "textarea", required: true,
        placeholder: "https://example.com/feed.xml\nhttps://other.com/rss",
        help: "One URL per line. HTTP Basic auth can be embedded: https://user:pass@host/feed." },
    ],
    defaultIntervalMins: 60,
    minIntervalMins: 15,
