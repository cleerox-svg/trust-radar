-- PR-AW — Widen notifications.type CHECK for abuse-mailbox events
--
-- Adds two new event types:
--   abuse_mailbox_verdict          — tenant (or super_admin) audience, fired
--                                    by the AI classifier when a capture
--                                    lands a high/critical phishing or malware
--                                    verdict. Includes a link to the message
--                                    detail UI.
--   abuse_mailbox_flood_detected   — super_admin audience, fired by the email
--                                    handler when the rate-limit decision
--                                    fires (per-sender or per-domain). Once
--                                    per (sender|domain) per hour via
--                                    group_key dedup.
--
-- Both events are registered in packages/shared/src/notification-events.ts.
-- They are system events (userToggleable: false) — abuse signals are not
-- engagement-grade noise that users opt out of, they're operational.
--
-- SQLite doesn't support ALTER TABLE ... DROP CONSTRAINT, so widening a
-- CHECK requires the same recreate dance as 0127 / 0128. The table swap
-- is safe — read paths consult column names, not constraint identity.

CREATE TABLE notifications_with_abuse_mailbox (
  id              TEXT PRIMARY KEY,
  ...
  type            TEXT NOT NULL CHECK (type IN (
                    ...
                    'abuse_mailbox_verdict',          -- NEW in PR-AW
                    'abuse_mailbox_flood_detected',   -- NEW in PR-AW
                    'notification_digest'
                  )),
...
INSERT INTO notifications_with_abuse_mailbox
SELECT * FROM notifications;

DROP TABLE notifications;
ALTER TABLE notifications_with_abuse_mailbox RENAME TO notifications;

-- Recreate the indexes (identical to 0128).
