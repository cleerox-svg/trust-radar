...
async function render() {
  ...
  if (route.auth) {
    ...
    if (mainEl) mainEl.insertAdjacentHTML('beforeend', '<div style="text-align:center;padding:16px;border-top:1px solid rgba(200,60,60,.15);font-size:10px;color:#4a5a73">Operated by <span style="color:#7a8ba8">LRX Enterprises Inc.</span> <svg width="20" height="15" viewBox="0 0 36 36" style="vertical-align:-3px;margin:0 3px"><rect width="36" height="36" rx="4" fill="#C83C3C"/><rect x="9" width="18" height="36" fill="#fff"/><path d="M18 6.6l-1.1 3.4-3.5-.2 2 2.6-1.6 1 2.3 1.2-.5 3.4L18 16.4l2.4 1.6-.5-3.4 2.3-1.2-1.6-1 2-2.6-3.5.2L18 6.6z" fill="#C83C3C"/><path d="M14.5 21h7v3h-7z" fill="#C83C3C"/></svg> Canadian owned and operated</div>');
...
function viewLogin(el) {
  el.innerHTML = `<div class="login-screen">
    ...
    <p style="color:var(--text-secondary);font-size:13px">Threat Interceptor</p>
    ...
    <div style="text-align:center;padding:16px 0 0;font-size:10px;color:var(--text-secondary)">Operated by <span style="color:var(--text-tertiary)">LRX Enterprises Inc.</span> <svg width="20" height="15" viewBox="0 0 36 36" style="vertical-align:-3px;margin:0 3px"><rect width="36" height="36" rx="4" fill="#C83C3C"/><rect x="9" width="18" height="36" fill="#fff"/><path d="M18 6.6l-1.1 3.4-3.5-.2 2 2.6-1.6 1 2.3 1.2-.5 3.4L18 16.4l2.4 1.6-.5-3.4 2.3-1.2-1.6-1 2-2.6-3.5.2L18 6.6z" fill="#C83C3C"/><path d="M14.5 21h7v3h-7z" fill="#C83C3C"/></svg> Canadian owned and operated</div>
  </div>`;
}
  ...
  el.innerHTML = `
    ...
    <footer class="pub-footer">
      ...
      <div class="pub-footer-legal">
        <div>\u00A9 2026 LRX Enterprises Inc. <svg width="20" height="15" viewBox="0 0 36 36" style="vertical-align:-3px;margin:0 3px"><rect width="36" height="36" rx="4" fill="#C83C3C"/><rect x="9" width="18" height="36" fill="#fff"/><path d="M18 6.6l-1.1 3.4-3.5-.2 2 2.6-1.6 1 2.3 1.2-.5 3.4L18 16.4l2.4 1.6-.5-3.4 2.3-1.2-1.6-1 2-2.6-3.5.2L18 6.6z" fill="#C83C3C"/><path d="M14.5 21h7v3h-7z" fill="#C83C3C"/></svg> Canadian owned and operated</div>
        <div>averrow.com</div>
...
async function viewAdminAgentConfig(el) {
  ...
  try {
    ...
    document.getElementById('adm-agent-config-table').innerHTML = agents.length ? `<div class="adm-table-scroll"><table class="adm-table"><thead><tr><th>Agent</th><th>Status</th><th>Schedule</th><th>Last Run</th><th>Avg Duration</th><th>Success Rate</th><th>Outputs (24h)</th><th>Actions</th></tr></thead><tbody>${agents.map(a => {
      ...
      return `<tr style="--agent-accent:${meta.color}" data-agent-color><td><div style="display:flex;align-items:center;gap:8px;border-left:3px solid ${meta.color};padding-left:10px"><div class="agent-icon ${meta.iconClass || ''}" style="width:28px;height:28px;display:flex;align-items:center;justify-content:center;color:${meta.color};background:${meta.color}15">${agentIcon(aid, 20)}</div><span style="font-weight:500;color:${meta.color}">${meta.label || a.display_name || a.name}</span></div></td><td><span class="adm-status-pill ${a.status || 'idle'}">${a.status || 'idle'}</span></td><td><span class="adm-schedule-pill">${cfg.schedule_label || a.schedule || cfg.schedule || '-'}</span></td><td style="font-family:var(--font-mono);font-size:10px;color:var(--text-tertiary)">${a.last_run_at ? relativeTime(Math.round((Date.now() - new Date(a.last_run_at).getTime()) / 60000)) : '-'}</td><td style="font-family:var(--font-mono);font-size:10px;color:var(--text-secondary)">${avgDur}</td><td style="font-family:var(--font-mono);font-size:10px;color:${parseFloat(successRate) >= 99 ? 'var(--positive)' : 'var(--threat-medium)'}">${successRate}%</td><td style="font-family:var(--font-mono);font-size:12px;font-weight:500">${a.outputs_24h || 0}</td><td style="white-space:nowrap"><button class="adm-action-btn adm-trigger-btn" data-agent="${a.agent_id || a.name}">Trigger Now</button></td></tr>`;
