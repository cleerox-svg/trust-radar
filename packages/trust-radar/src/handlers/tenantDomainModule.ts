...
export async function handleGetDomainModuleSummary(
  ...
  const result = await env.DB.prepare(
    `SELECT
       ...
       (SELECT COUNT(*) FROM lookalike_domains ld WHERE ld.brand_id = b.id AND ld.threat_level = 'CRITICAL') AS lookalikes_critical,
       (SELECT COUNT(*) FROM lookalike_domains ld WHERE ld.brand_id = b.id AND ld.threat_level = 'HIGH') AS lookalikes_high,
