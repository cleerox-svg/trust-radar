...
export function renderPlatformFeedAutoPaused(v: PlatformFeedAutoPausedVars): RenderedTemplate {
  return {
    ...
    reason_text: `Platform alert — operational only.`,
    recommended_action: `Manual unpause via /admin/feeds once the root cause is fixed.`,
    link: '/admin/feeds',
...
export function renderPlatformAgentStalled(v: PlatformAgentStalledVars): RenderedTemplate {
  return {
    ...
    reason_text: `Platform alert — operational only.`,
    recommended_action: `Force-fail the run via /admin/agents; the next tick will re-dispatch. Investigate the stall reason in agent_runs.`,
    link: '/admin/agents',
