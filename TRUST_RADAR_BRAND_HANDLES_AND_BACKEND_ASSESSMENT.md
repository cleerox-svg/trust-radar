...
│         (AI calls now go directly to Anthropic API               │
...
│              │ PRIMARY       │ INDEPENDENT   │                   │
├──────────────┼───────────────┼───────────────┼───────────────────┤
│ AI calls     │ ✓ Direct to   │ ? Via API?    │ ✓ Anthropic/OAI   │
│ (inference)  │ Anthropic API │               │ LIKELY OBSOLETE   │
├──────────────┼───────────────┼───────────────┼───────────────────┤
...
**Problem:** The `packages/api` FastAPI service was originally a shared backend for AI inference (enhance-bio, scan-insight, impression-report). Both Workers now call Anthropic directly, making FastAPI a dead middleman that adds latency and hosting cost.
...
IF references still exist:
  - Migrate those calls to direct Anthropic API calls from Workers
...
**Action for Claude Code:**
```
1. Audit all files that import/call Anthropic API
...
BACKEND STEP 2: Create Unified AI Client
  ...
  └─ Audit all Anthropic API call sites
...
│
├── lib/
│   ├── ai-client.ts            ← Unified Anthropic API client
