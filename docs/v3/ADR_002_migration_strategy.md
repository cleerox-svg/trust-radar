...
The "one-shot port at cutover" rows go through a **migration script** (`packages/averrow-worker/scripts/migrate-tenant-to-v3.ts`) that:
...
- v2 platform reference:
  - `packages/averrow-worker/src/agents/flightControl.ts` — self-monitor pattern v3 dashboards inherit
  - `packages/averrow-worker/src/lib/cube-builder.ts` — cube rebuild pattern v3 reuses for actor cubes
