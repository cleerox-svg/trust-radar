...
| POST | `/api/internal/auth/mint-ui-preview-jwt` | Mint a SHORT-LIVED, LOW-PRIVILEGE JWT for Claude Code UI inspection. Params: `surface=staff\|tenant` (required); staff `role=analyst\|admin` (default analyst, never super_admin); tenant `org_id=N` (optional, scopes to a real org); `ttl_minutes=N` (default 60, max 240). Returns `{ jwt, preview_url, expires_at, ttl_seconds, user_id, role, surface, org_id }`. Load `preview_url` (`…/v2/#token=…` or `…/tenant/#token=…`) in a browser to boot the SPA. Dedicated users `claude_ui_staff` / `claude_ui_tenant`. **Kill switch:** `UPDATE users SET status='suspended' WHERE id IN ('claude_ui_staff','claude_ui_tenant');` |
