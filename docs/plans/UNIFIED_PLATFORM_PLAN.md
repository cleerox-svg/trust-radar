...
Trust Radar is an **AI-powered brand threat intelligence platform** that provides outside-in brand protection monitoring — works instantly with zero setup — with the autonomous capability to connect to customer platforms for deeper signal and automated response when available.

### Tagline Options (pick one during implementation)

- "AI-Powered Brand Threat Intelligence"
- "See Your Brand the Way Attackers Do"
- "Brand Protection Intelligence. AI-Powered. Autonomous."
- "Outside-In Intelligence. Inside-Out Action."

### Core Value Proposition

> Trust Radar continuously monitors the internet for brand impersonation, phishing infrastructure, email security vulnerabilities, and social media abuse targeting your organization — powered by AI agents that analyze threats and deliver actionable intelligence. It works instantly with zero integration, but can connect to your existing security platforms (email gateways, SIEM, identity providers) for deeper signal and autonomous response.

### Platform Philosophy

Trust Radar operates in two modes:

**1. Outside-In (default, zero setup)**
Scans the open internet, DNS infrastructure, social platforms, and threat intelligence feeds to build a complete picture of your brand's exposure. No agents, no credentials, no integration required. Works out of the box.

**2. Connected (optional, deeper signal)**
Optionally integrates with customer platforms for enhanced intelligence and autonomous action:
- Email security gateways (e.g., secure email gateways, cloud email security) for real-time email threat signal
- SIEM/SOAR platforms for automated incident creation and response workflows
- Identity providers for credential exposure correlation
- Communication platforms for alert delivery and team coordination

> **TODO:** Build the Connected integration framework. Define integration API patterns, OAuth flows for customer platform access, and the autonomous action engine. This is a Phase 8+ capability that differentiates from purely-passive competitors.

The differentiator: competitors are either purely passive monitors (outside-in only, can't take action) or require deep integration to function at all. Trust Radar works immediately at full capability, and gets even better when connected.

### Competitive Positioning

Trust Radar is NOT:
- An enterprise DRP platform ($20K-$150K+/yr from incumbent vendors)
- A marketplace counterfeit tool
...
Body: Trust Radar performs deep outside-in analysis of SPF, DKIM, and DMARC
      ...
      over time. With optional email gateway integration, gain real-time signal
      from your actual email security platform for even deeper visibility.
Visual: Example email security report card (A through F grading)
        Before/after showing improvement
```

> **TODO:** Build email gateway integration (Connected mode). Define OAuth/API patterns for
> connecting to customer email security platforms. This enables real-time threat signal
> beyond what outside-in DNS analysis provides.
```
...
Tier 4: "Enterprise" — Custom (starting $4,999/month)
...
Bottom: "© 2026 LRX Enterprises Inc. All rights reserved."
Badges: Cloudflare | SOC 2 (Planned)
```
...
│  ├─ Phishing DB: 1 active entry                   │
│  ├─ Malware feeds: 0                                  │
│  └─ Threat URLs: 1 historical                     │
│                                                 │
...
│   phishing database entry targeting this domain suggests │
│   it has already been targeted. Recommend         │
│   immediate DKIM selector expansion and           │
│   monitoring activation."                         │
│                                                 │
│  [Monitor This Brand — $799/mo]  [Share Report]  │
│                                                 │
└─────────────────────────────────────────────────┘
```
...
POST /api/scan/report (public, rate-limited)
  ...
  Process:
    ...
    2. Threat feed check (existing pipeline — phishing databases, malware feeds, and threat URL sources)
    ...
    5. AI assessment via Analyst agent (AI engine)
...
Trust Radar is a brand threat intelligence platform that starts outside-in — monitoring the open internet for impersonation, phishing, email vulnerabilities, and social media abuse — with the autonomous capability to connect to customer security platforms for deeper signal and automated response.
...
- **AI Threat Analysis** — AI-powered Analyst and Observer agents that correlate signals and generate human-readable threat narratives
- **Email Security Posture Engine** — Outside-in SPF/DKIM/DMARC assessment with grading and historical tracking
- **Threat Feed Integration** — Aggregates phishing databases, malware URL feeds, certificate transparency logs, and credential breach intelligence
...
└──────────────────────────────────────────────────────┘
    ...
    │ (AI)       │           │ (Cron/30min) │
    │ Analyst    │           │ Phishing DBs  │
    │ Observer   │           │ Malware Feeds  │
    │ Sales*     │           │ Breach Intel   │
    └────────────┘           │ CT Logs      │
                             └──────────────┘
```
...
| AI | Advanced AI Engine (configured via env) | Agent reasoning and analysis |
...
- [x] Threat feed integration (phishing databases, malware feeds, and threat URL sources)
...
- Remove any references to "OpenAI GPT-4o-mini" in the stack — the platform uses AI engine
...
#### 0c. Stack Cleanup
- [ ] Remove "OpenAI GPT-4o-mini" references — replace with "Advanced AI (configured via AI_API_KEY)"
...
- [ ] Integrate CT log polling (certificate transparency log APIs)
...
- [ ] credential breach database search with stealer log access (~$30-50/month)
...
- [ ] Reason about specific feed details (e.g., "sender IP hosts 3 known phishing pages")
...
- Corporate site for LRX Enterprises Inc.
...
chore(cleanup): remove OpenAI references, update to AI engine
```
...
|------|-------|-----------|-------------|
| Scan (Free) | $0 | ~$0.002/scan (AI API call + DNS lookups). Budget 1000 free scans/month = ~$2 | Acquisition tool |
| Professional | $799/mo | Credential breach API (~$30-50/mo) + social monitoring API access ($20-40/mo) + AI inference (~$15-25/mo per brand) + infrastructure (~$5/mo) = ~$70-120/mo per customer | ~85% margin |
| Business | $1,999/mo | Up to 10 brands. Feed costs shared across brands. AI inference scales (~$50-80/mo). CT monitoring + STIX export = minimal marginal cost | ~90%+ margin |
| Enterprise | Custom (starting $4,999/mo) | Multi-tenant overhead, SSO, SLA, dedicated support, custom agent tuning | ~90%+ margin |

**Feed subscription costs that drive the pricing floor:**
- Credential breach intelligence + stealer log access: ~$30-50/month
...
**Pricing Strategy — 1/2 to 2/3 of incumbent platforms:**
Established brand protection and digital risk protection platforms price between $20,000 and $150,000+ per year. Trust Radar targets the 1/2 to 2/3 price band:

| Incumbent Range | Trust Radar Equivalent | Ratio |
|----------------|----------------------|-------|
| $20K-$30K/yr (entry) | Professional: $9,588/yr | ~1/2 |
| $30K-$50K/yr (mid) | Business: $23,988/yr | ~1/2 to 2/3 |
| $60K-$120K/yr (upper) | Enterprise: $59,988+/yr | ~1/2 |

This positioning communicates serious enterprise capability (not a toy) while delivering meaningful savings. AI agents replacing human analyst teams is what makes the economics work — Trust Radar scales per-customer cost with compute, not headcount.
